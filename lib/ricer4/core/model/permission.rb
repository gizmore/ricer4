module Ricer4
  class Permission
    ...
    def to_label
      I18n.t("ricer4.permission.#{self.name}")
    end
    ...
    def display_symbols(respect_auth=REGISTERED)
      out = ''
      bits = self.all_bits(respect_auth)
      ALL.each do |p|
        if (p.symbol) && ((p.bit & bits) > 0)
          out += p.symbol
        end
      end
      out
    ...
    # def self.by_symbol(symbol)
      # ALL.each{|p| return p if p.symbol == symbol }
      # nil
    # end
    ...
    def self.bits_from_symbols(symbols)
      bits = 0
      ALL.each do |p|
        bits |= p.bit if (p.symbol != '') && (symbols.index(p.symbol))
      end
      bits
    end
    ...
    def self.by_nickname(nickname, authenticated=false)
      self.by_permission(self.bits_from_nickname(nickname), authenticated)
    end
    
    def self.by_permission(permissions, authenticated=false)
      priv = 'p'; symbol = ''; char = ''; bit = 0; name = :public
      ALL.each do |p|
