...
#    ALL        ||= new(char:'a', bit:0x0F000000, :name => :all)    # For settings only
    @@all = { user:USER, channel:CHANNEL, everywhere:EVERYWHERE, bot:BOT, server:SERVER }
    ...
    def self.by_name!(name)
      by_name(name) or raise StandardError.new("Invalid scope: #{name}")
    end
    ...
    def to_label; I18n.t('ricer3.scope.'+self.char); end
    def to_usage_label; I18n.t('ricer3.scope.usage.'+self.char); end
    
    def everywhere?; (self.bit & EVERYWHERE.bit) == EVERYWHERE.bit; end
    def channel?; (self.bit & CHANNEL.bit) > 0; end
    def private?; (self.bit & USER.bit) > 0; end
