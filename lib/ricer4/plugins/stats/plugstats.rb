module Ricer4::Plugins::Stats
  class Plugstats < Ricer4::Plugin
    ...
    def plugin_init
      ...
      arm_subscribe('ricer/triggered') do |plugin|
    ...
    def execute_show_user(plugin, user)
      rply :one_plugin_and_user, plugin:plugin.trigger, user:user.display_name, count:TriggerCounter.for_plugin(plugin).for_user(user).first.calls
    end
