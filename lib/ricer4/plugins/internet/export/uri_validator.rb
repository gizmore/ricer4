class UriValidator < ActiveModel::EachValidator
  ...
  def default_options
    { ping: false, exists: false, connect: false, ssl_trust: false, schemes: [:http, :https] }
  end
  ...
  def ping?; @options[:ping]; end
  def exists?; @options[:exists]; end
  def connect?; @options[:connect]; end
  def trusted?; @options[:ssl_trust]; end
  ...
  def schemes; @options[:schemes]; end
