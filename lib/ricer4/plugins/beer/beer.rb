module Ricer4::Plugins::Beer
  class Beer < Ricer4::Plugin
    ...
    has_usage  '', function: :execute_fetch_one
    def execute_fetch_one
      return beer_alert if beer_empty?
      arply(:msg_give_beer, drinker: sender.display_name, left: beer_left)
    end

    has_usage  '<user|current_channel:true>', function: :execute_give_beer
    def execute_give_beer(drinker)
      return execute_fetch_one if drinker == sender
      return beer_alert if beer_empty?
      return beer_stolen! if beer_stolen?
      arply(:msg_give_beer, giver: sender.display_name, drinker: drinker.display_name, left: beer_left)
    end
    ...
    def beer_stolen!(drinker)
      ...
      arply :msg_stolen, stealer: stealers.sample.display_name, drinker: drinker.display_name, left: beer_left_text 
    end
