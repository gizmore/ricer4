module Ricer4::Plugins::Irc
  class IrcUsers < Ricer4::Plugin
    ...
    def plugin_init
      arm_subscribe('irc/nick') { |message| on_user_nick(message) }
      arm_subscribe('irc/quit') { |message| on_user_quit(message) }
      arm_subscribe('irc/mode') { |message| on_user_mode(message) }
    end
